...
describe('buildH3Fl2vaWorkflow', () => {
  ...
  it('injects model files, sampling, and output settings', async () => {
    ...
    expect(wf[H3_FL2VA.UNET_LOADER]!.inputs).toMatchObject({ unet_name: 'test-h3-unet.safetensors', weight_dtype: 'default' })
    expect(wf[H3_FL2VA.CLIP_LOADER]!.inputs).toMatchObject({ clip_name: 'test-h3-clip.safetensors', type: 'test-clip-type', device: 'default' })
    ...
    expect(wf[H3_FL2VA.SAMPLER_SELECT]!.inputs!.sampler_name).toBe('euler')
    expect(wf[H3_FL2VA.SCHEDULER]!.inputs!.scheduler).toBe('simple')
    ...
    expect(wf[H3_FL2VA.VIDEO_COMBINE]!.inputs).toMatchObject({ crf: 18, format: 'video/h264-mp4', pix_fmt: 'yuv420p', filename_prefix: 'H3FL2VA/first' })
  })
  ...
  it('keeps RTX node wired when enabled', async () => {
    const wf = await buildH3Fl2vaWorkflow({ model: 'h3-fl2va', prompt: 'p', videoDuration: 5, inputImage: 'first.png' })
    expect(wf[H3_FL2VA.RTX_SUPER_RES]!.inputs).toMatchObject({ resize_type: 'scale by multiplier', 'resize_type.scale': 2, quality: 'HIGH' })
