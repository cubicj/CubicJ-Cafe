...
const mockGetH3Ref2vaSettings = vi.fn()
...
vi.mock('@/lib/database/system-settings', () => ({
  ...
  getH3Ref2vaSettings: (...args: unknown[]) => mockGetH3Ref2vaSettings(...args),
}))
...
describe('model settings adapter', () => {
  ...
  it('loads settings with the model-keyed loader', async () => {
    ...
    const h3Ref2vaSettings = { durationOptions: [10] }
    ...
    mockGetH3Ref2vaSettings.mockResolvedValue(h3Ref2vaSettings)
    ...
    await expect(getModelSettings('h3-ref2va')).resolves.toBe(h3Ref2vaSettings)
  })
  ...
  it('converts LTX frame counts to rounded seconds', () => {
    ...
    expect(getVideoDurationSeconds('h3-ref2va', 7, { framesPerStep: 8, frameBase: 2, frameRate: 10 })).toBe(5.8)
  })
  ...
  it('preserves duration values for other models or missing frame settings', () => {
    ...
    expect(getVideoDurationSeconds('h3-ref2va', 7, {})).toBe(7)
  })
})
