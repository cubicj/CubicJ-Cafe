...
describe('buildH3Ref2vaWorkflow', () => {
  ...
  it('wires image references through per-slot resize nodes', async () => {
    ...
    expect(wf[refImageResizeId(0)]!.inputs).toMatchObject({ megapixels: 0.5, multiple_of: 16, upscale_method: 'lanczos', image: [refImageLoadId(0), 0] })
  ...
  it('wires videos and soundtracks by slot', async () => {
    ...
    expect(wf[refVideoResizeId(0)]!.inputs).toMatchObject({ megapixels: 0.3, multiple_of: 16, upscale_method: 'lanczos', image: [refVideoLoadId(0), 0] })
  ...
  it('injects model files, sampling, and output settings', async () => {
    ...
    expect(wf[H3_REF2VA.UNET_LOADER]!.inputs).toMatchObject({ unet_name: 'test-h3r-unet.safetensors', weight_dtype: 'default' })
    expect(wf[H3_REF2VA.TURBO_LORA]!.inputs).toMatchObject({ lora_name: 'test-h3r-lora.safetensors', strength_model: 0.9 })
    expect(wf[H3_REF2VA.CLIP_LOADER]!.inputs).toMatchObject({ clip_name: 'test-h3r-clip.safetensors', type: 'test-clip-type', device: 'default' })
    ...
    expect(wf[H3_REF2VA.SAMPLER_SELECT]!.inputs!.sampler_name).toBe('euler')
    expect(wf[H3_REF2VA.SCHEDULER]!.inputs!.scheduler).toBe('simple')
    ...
    expect(wf[H3_REF2VA.VIDEO_COMBINE]!.inputs).toMatchObject({ crf: 18, format: 'video/h264-mp4', pix_fmt: 'yuv420p', filename_prefix: 'H3Ref2VA/img0' })
  })
  ...
  it('keeps RTX node wired when enabled', async () => {
    const wf = await buildH3Ref2vaWorkflow(baseParams())
    expect(wf[H3_REF2VA.RTX_SUPER_RES]!.inputs).toMatchObject({ resize_type: 'scale by multiplier', 'resize_type.scale': 2, quality: 'HIGH' })
