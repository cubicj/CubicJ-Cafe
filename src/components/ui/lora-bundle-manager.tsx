...
export function LoRABundleManager() {
  ...
  const fetchBundles = async () => {
    ...
    try {
      ...
      console.log('✅ LoRA bundle list fetched:', data.count, 'items');
    } catch (err) {
  ...
  const fetchAvailableLoRAs = async () => {
    ...
    try {
      ...
      console.log('✅ LoRA file list fetched:',
      ...
      if (err instanceof Error && !err.message.includes('503') && !err.message.includes('Service Unavailable')) {
        console.error('❌ Failed to fetch LoRA file list:', err);
      }
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.log(`✅ LoRA bundle ${editingBundle ? 'updated' : 'created'} successfully`);
      ...
      console.error('❌ Failed to save LoRA bundle:', err);
  ...
  const handleDelete = async (bundleId: string) => {
    ...
    try {
      ...
      console.log('✅ LoRA bundle deleted successfully');
      
      // 목록 새로고침
      await fetchBundles();
      
    } catch (err) {
      console.error('❌ Failed to delete LoRA bundle:', err);
