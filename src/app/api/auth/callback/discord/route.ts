...
export async function GET(request: NextRequest) {
  ...
  log.info('Discord callback params', { error, code: code ? code.substring(0, 10) + '...' : null });
  ...
  if (code) {
    try {
      ...
      try {
        ...
        log.error('Network error during token exchange', { error: fetchError instanceof Error ? fetchError.message : String(fetchError) });
